...
class RootTabBarController : UITabBarController
{
    ...
    func configureLayout()
    {
        ...
        //定制标签栏
        self.tabBarView = UIView(frame: CGRectMake(0, self.view.bounds.height - 49, self.view.bounds.width, 49))
        self.tabBarView?.backgroundColor = UIColor(red: 125.0 / 255.0, green: 236 / 255.0, blue: 198 / 255.0, alpha: 1)
        self.view.addSubview(self.tabBarView!)
        
        //滑块
        let width = self.view.bounds.width / 4.0
        ...
        for item in barItemsArray
        {
            ...
            button.setTitleColor(UIColor(red: 52 / 255.0, green: 156 / 255.0, blue: 150 / 255.0, alpha: 1), forState: UIControlState.Selected)
    ...
    func onTabBarItemButtonClicked(sender: UIButton!)
    {
        ...
        UIView.animateWithDuration(0.3, animations: { () -> Void in
            //所有类型都要求匹配，不允许作何不匹配的情况
            var x = CGFloat(self.selectedIndex) * self.view.bounds.width
